...
```
/agentic:implement path/to/plan.md         → read file
/agentic:implement <text with plan>        → use as-is
/agentic:implement                         → check conversation context for plan
```
...
> - **`/agentic:technical-planning`** — interactive workflow that produces a plan from any spec/idea
...
>
> Then re-run `/agentic:implement` with the plan.
...
**Set output path:**
```yaml
output_path: "{ide-folder}/{outputFolder}/task/implement/{topic}/{instance_id}"
```
