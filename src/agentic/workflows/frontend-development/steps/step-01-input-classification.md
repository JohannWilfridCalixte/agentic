...
```
/agentic:workflow:frontend-development "Build a dashboard"    → use description
/agentic:workflow:frontend-development path/to/spec.md       → read spec file
/agentic:workflow:frontend-development https://figma.com/... → fetch design reference
/agentic:workflow:frontend-development                        → ERROR: input required
```
...
```bash
mkdir -p {ide-folder}/{outputFolder}/frontend/{session_id}
```
...
session_path: {ide-folder}/{outputFolder}/frontend/{session_id}
