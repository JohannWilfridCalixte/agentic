...
while IFS= read -r -d '' file; do
    VISION+=("$file")
done < <(find documentation/product/vision -name "*.md" -print0 2>/dev/null)
while IFS= read -r -d '' file; do
    VISION+=("$file")
done < <(find documentation/tech/vision -name "*.md" -print0 2>/dev/null)
while IFS= read -r -d '' file; do
    VISION+=("$file")
done < <(find documentation/tech/dx -name "*.md" -print0 2>/dev/null)

# Phase 2: Epics
while IFS= read -r -d '' file; do
    EPICS+=("$file")
done < <(find documentation/product/prd -name "epic.md" -print0 2>/dev/null)

# Phase 3: User Stories
while IFS= read -r -d '' file; do
    USER_STORIES+=("$file")
done < <(find documentation/product/prd -name "US-*.md" -print0 2>/dev/null)

# Phase 4: Task docs (technical-context, technical-plan, security-addendum)
while IFS= read -r -d '' file; do
    TASK_DOCS+=("$file")
done < <(find documentation/task -name "technical-context.md" -print0 2>/dev/null)
while IFS= read -r -d '' file; do
    TASK_DOCS+=("$file")
done < <(find documentation/task -name "technical-plan.md" -print0 2>/dev/null)
while IFS= read -r -d '' file; do
    TASK_DOCS+=("$file")
done < <(find documentation/task -name "security-addendum.md" -print0 2>/dev/null)
